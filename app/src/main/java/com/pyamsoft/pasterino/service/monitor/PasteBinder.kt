...
internal constructor(
  ...
  private val pasteRunner =
      highlander<PasteEvent> {
  ...
  private inline fun CoroutineScope.listenPaste(
      crossinline onEvent: (event: ServiceEvent) -> Unit
  ) =
      launch(context = Dispatchers.Default) {
        pasteRequestBus.onEvent {
          val pasteEvent = pasteRunner.call()
          withContext(context = Dispatchers.Main) { onEvent(pasteEvent) }
        }
      }

  internal fun handleStart(scope: CoroutineScope) {
    scope.launch(context = Dispatchers.Default) { interactor.setServiceState(true) }
  }

  internal fun handleStop(scope: CoroutineScope) {
    scope.launch(context = Dispatchers.Default) { interactor.setServiceState(false) }
  }
